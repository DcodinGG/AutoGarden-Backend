// ai.js — Watering decision engine powered by Google Gemini
...
// System instruction for the decision engine
const SYSTEM_PROMPT = `You are the intelligent irrigation system of AutoGarden. Your task is to decide whether a plant should be watered now and for how long, considering all available factors.
...
## Response
Reply ONLY with a valid JSON object with this exact structure:
${RESPONSE_SCHEMA}`;

// Builds the context portion of the prompt with live sensor/context data
function buildContextPrompt({ plant, moisture, ambientTemp, ambientHumidity, weather, history }) {
...
function quickDecision(moisture, weather) {
  ...
  return null; // No shortcut — let Gemini decide
}
...
async function getWateringDecision({ plant, moisture, ambientTemp, ambientHumidity, weather, history }) {
  ...
  const contextPrompt = buildContextPrompt({ plant, moisture, ambientTemp, ambientHumidity, weather, history });
  ...
  try {
    ...
    console.log(`[ai] Gemini raw response for plant ${plant?.name || '?'}: ${raw}`);
    ...
    console.error('[ai] Error processing Gemini response:', err.message);
    return fallbackDecision(moisture);
  }
}
...
function fallbackDecision(moisture) {
  ...
  return {
    ...
    alert: 'Gemini API unavailable. Using fixed moisture thresholds.'
  };
}
